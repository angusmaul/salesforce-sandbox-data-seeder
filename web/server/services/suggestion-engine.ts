...
export interface SuggestionRequest {
  ...
  relationshipContext?: {
    parentRecords?: Array<{objectName: string; recordData: any}>;
    relatedFields?: Array<{fieldName: string; value: any}>;
  };
  validationRules?: any[];
  existingData?: any[];
  recordIndex?: number;
...
export interface SuggestionMetrics {
  ...
  fieldTypeBreakdown: {[fieldType: string]: {total: number; accepted: number}};
  businessContextUsage: {[context: string]: number};
}
...
export class SuggestionEngine {
  ...
  private industryPatterns: {[industry: string]: any} = {};
  private suggestionCache: Map<string, FieldSuggestion[]> = new Map();
   ...
   */
  async generateFieldSuggestions(request: SuggestionRequest): Promise<FieldSuggestion[]> {
   ...
   */
  private async generateAISuggestions(request: SuggestionRequest): Promise<FieldSuggestion[]> {
...
Record Index: ${recordIndex + 1}`;
    ...
    if (fieldMetadata) {
      ...
      if (fieldMetadata.picklistValues?.length > 0) {
        prompt += `\n- Picklist Options: ${fieldMetadata.picklistValues.slice(0, 5).map((p: any) => p.value).join(', ')}${fieldMetadata.picklistValues.length > 5 ? '...' : ''}`;
      }
    }
...
- businessContext: what business scenario this represents`;
   ...
   */
  private parseAISuggestionResponse(response: string, request: SuggestionRequest): FieldSuggestion[] {
    try {
   ...
   */
  private extractSuggestionsFromText(response: string, request: SuggestionRequest): FieldSuggestion[] {
   ...
   */
  private validateAndFormatValue(value: any, fieldType: string, fieldMetadata?: any): any {
    try {
      switch (fieldType.toLowerCase()) {
        ...
        case 'picklist':
          if (fieldMetadata?.picklistValues?.length > 0) {
            ...
            const validOption = fieldMetadata.picklistValues.find((p: any) => 
   ...
   */
  private async enhanceSuggestions(suggestions: FieldSuggestion[], request: SuggestionRequest): Promise<FieldSuggestion[]> {
   ...
   */
  private applyIndustryPatterns(suggestions: FieldSuggestion[], request: SuggestionRequest): FieldSuggestion[] {
   ...
   */
  private applyRelationshipLogic(suggestions: FieldSuggestion[], request: SuggestionRequest): FieldSuggestion[] {
    ...
    // Example: If Opportunity.Account is set, suggest realistic close dates based on deal size
    if (request.objectName === 'Opportunity' && request.fieldName === 'CloseDate') {
      const accountRecord = context.parentRecords?.find(p => p.objectName === 'Account');
      const amount = context.relatedFields?.find(f => f.fieldName === 'Amount')?.value;
      
      if (amount && typeof amount === 'number') {
        // Larger deals typically have longer sales cycles
        const daysToClose = amount > 100000 ? 90 : amount > 50000 ? 60 : 30;
        const closeDate = new Date();
        closeDate.setDate(closeDate.getDate() + daysToClose);
        
        suggestions.forEach(suggestion => {
          suggestion.value = closeDate.toISOString().split('T')[0];
          suggestion.confidence = Math.min(suggestion.confidence * 1.1, 1.0);
          suggestion.reasoning += ` (${daysToClose}-day cycle for deal size)`;
        });
      }
    }
   ...
   */
  private generateFallbackSuggestions(request: SuggestionRequest): FieldSuggestion[] {
    ...
    basicValues.forEach((value: any, index: number) => {
      suggestions.push({
        value,
   ...
   */
  recordSuggestionInteraction(suggestionId: string, action: 'accepted' | 'rejected' | 'modified', modifiedValue?: any) {
    switch (action) {
      case 'accepted':
   ...
   */
  getBusinessScenarios(): BusinessScenario[] {
  ...
  private initializeMetrics(): SuggestionMetrics {
    return {
      totalSuggestions: 0,
  ...
  private updateMetrics(action: string, count: number, request: SuggestionRequest) {
    if (action === 'generated') {
  ...
  private initializeBusinessScenarios(): BusinessScenario[] {
    return [
      {
  ...
  private matchesIndustryPattern(value: any, patterns: any): boolean {
    if (!patterns.valueTypes) return false;
    
    const valueStr = String(value).toLowerCase();
    return patterns.valueTypes.some((pattern: string) => 
