...
export class SandboxService {
  ...
  async detectSandboxInfo(): Promise<SandboxInfo> {
    try {
  ...
  private determineSandboxType(dataLimitMB: number, fileLimitMB: number): SandboxInfo['type'] {
    // Determine sandbox type based on storage limits
  ...
  async calculateAvailableStorage(sandboxInfo: SandboxInfo): Promise<{
  ...
  async estimateRecordCapacity(sandboxInfo: SandboxInfo, avgRecordSizeKB: number = 2): Promise<{
    maxRecords: number;
  ...
  async validateStorageBeforeGeneration(
    sandboxInfo: SandboxInfo, 
    estimatedRecords: number, 
    avgRecordSizeKB: number = 2
  ): Promise<{ canProceed: boolean; warnings: string[]; errors: string[] }> {
  ...
  getRecommendedObjectCounts(sandboxInfo: SandboxInfo, objectCount: number): { [key: string]: number } {
  ...
  private getBaseRecordsPerObject(sandboxInfo: SandboxInfo): number {
